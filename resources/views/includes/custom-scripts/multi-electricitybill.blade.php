<script type="text/javascript">
  $(document).ready(function() {
    $("#electricitybilltable").on('click.a[href="#_js"]', function() {
        return false;
    });
    ...
    $("#addrow").on("click", function () {
        ...
        cols += '<td><select name="electricity_meters[]" class="js-example-basic-single" style="width:100%;"><option disabled selected value>Select Unit/Meter</option>@foreach ($utilities->where('utility_type_id', 1) as $electricity_meter)<option value="{{ $electricity_meter->id }}">{{ $electricity_meter->no }} | {{ $electricity_meter->unit->number }} ({{ $electricity_meter->unit->status }})</option>@endforeach</select></td>';
        ...
        newRow.append(cols);
        console.log(cols)
        $("#electricitybilltable").append(newRow);
        counter++;
    ...
    $("#electricitybilltable").on("click", "#btnDel", function (event) {
